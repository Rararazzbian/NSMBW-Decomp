...
void daPyMng_c::checkCorrectCreateInfo() {
    for (int idx = 0; idx < 4; idx++) {
        PLAYER_TYPE_e type = mPlayerType[idx];
        if (type > 3) {
            type = (PLAYER_TYPE_e) 0;
            mPlayerType[idx] = type;
        }
        PLAYER_POWERUP_e mode = mPlayerMode[type];
        if (mode > 6) {
            mPlayerMode[type] = (PLAYER_POWERUP_e) 0;
        }
        if (mCreateItem[type] & 0xE) {
            mCreateItem[type] = mode & 1;
        }
        int rest = mRest[type];
        if (rest < 0 || rest > scRestMax) {
            rest = 5;
        }
        mRest[type] = rest;
    }
