...
test_cases = [
    # D1: deathData declared at top
    """sDeathInfoData deathData;
    ...
    mActorProperties &= ~8;
    deathData = l_dieQuake;
    deathData.mDirection = dir;
    mDeathInfo = deathData;""",

    # D2: deathData initialized at top
    ...
    # D3: dir saved in deathData directly
    """sDeathInfoData deathData = l_dieQuake;
    deathData.mDirection = getPl_LRflag(mPos);
    if (mAnmMatClr.mpChildren[1].getObj() != nullptr) {
        mAnmMatClr.setFrame(0.0f, 1);
    }
    removeCc();
    ...
    # D4: no deathData local at all, assign field by field or macro
    """u8 dir = getPl_LRflag(mPos);
    if (mAnmMatClr.mpChildren[1].getObj() != nullptr) {
        mAnmMatClr.setFrame(0.0f, 1);
    }
...
"""
    ...
    if d_fn:
        ...
        print(f"D{i+1}: T={len(t_fn)} D={len(d_fn)}, raw={raw_diffs}, can={diffs}")
        if raw_diffs == 0:
            print(f"*** D{i+1} MATCHES 100%! ***")
            break
