...
test_cases = [
    ...
    # C3: inline deathData copy with no local
    """u8 dir = getPl_LRflag(mPos);
    if (mAnmMatClr.mpChildren[1].getObj() != nullptr) {
        mAnmMatClr.setFrame(0.0f, 1);
    }
    ...
    mActorProperties &= ~8;
    (mDeathInfo = l_dieQuake).mDirection = dir;""",
]
...
"""
    ...
    if d_fn:
        ...
        print(f"C{i+1}: T={len(t_fn)} D={len(d_fn)}, raw={raw_diffs}, can={diffs}")
        if raw_diffs == 0:
            print(f"*** C{i+1} MATCHES 100%! ***")
            break
