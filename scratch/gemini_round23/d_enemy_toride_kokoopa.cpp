...
void dEnTorideKokoopa_c::setFumiDamage(dActor_c *killedBy) {
    ...
    if ((u32)playerNo <= 3) {
        dScoreMng_c::m_instance->ScoreSet(this, 5, playerNo, dScoreMng_c::smc_SCORE_X, dScoreMng_c::smc_SCORE_Y);
    }
...
void dEnTorideKokoopa_c::setFireDamage(dActor_c *killedBy) {
    mFireTimer = 60;
    if (mpBossLife->isDmgSection()) {
        if (mAnmMatClr.mpChildren[1].getObj() != nullptr) {
            mAnmMatClr.setFrame(0.0f, 1);
...
void dEnTorideKokoopa_c::setStarDamage(dActor_c *killedBy) {
    if (mAnmMatClr.mpChildren[1].getObj() != nullptr) {
        mAnmMatClr.setFrame(0.0f, 1);
    }
    removeCc();
...
mVec3_c dEnTorideKokoopa_c::calcBlitzPos() {
    mMdlKokoopa.getResMdl();
    u32 nodeIdx = createBlitz_sub();
    nw4r::math::MTX34 mtx;
    mMdlKokoopa.getNodeWorldMtx(nodeIdx, &mtx);
    mVec3_c offset = getMagicStickEffectOffset();
    mVec3_c res;
    PSMTXMultVec(mtx, offset, res);
    return res;
}
...
int dEnTorideKokoopa_c::calcDirAngle(short step) {
    return sLib::chaseAngle(&mAngle.y.mAngle, l_EnMuki[mDirection] * defaultDirAngle(), step);
}

int dEnTorideKokoopa_c::defaultDirAngle() {
    return 0x2000;
}
...
int dEnTorideKokoopa_c::getTorideFunfareTime() {
    return 40;
}

int dEnTorideKokoopa_c::getTurnSpeed() {
...
void dEnTorideKokoopa_c::wandCcCallback(dCc_c *self, dCc_c *other) {
    ...
    if (actor->mKind == 1) {
        ...
        if (!pl->isNoDamage()) {
            pl->setDamage(self->mpOwner, (daPlBase_c::DamageType_e)0);
        }
    }
}
...
void dEnTorideKokoopa_c::calcLookAngle() {
    dActor_c *player = daPyMng_c::getPlayer(mUnk764);
    float dx = player->mPos.x - mLookatPos.x;
    float dy = player->mPos.y - mLookatPos.y;
    s16 angle = cM::atan2s(dy, std::fabs(dx));
    mUnk792 = angle;
    if (angle < 0) {
        mUnk792 = 0;
...
int dEnTorideKokoopa_c::getDownTime() {
    return 50;
}

int dEnTorideKokoopa_c::getFumiRecoverTime() {
    return 4;
}
...
void dEnTorideKokoopa_c::notice1Vo() {
    if (mpParamDemo->mAnmNames[1] != nullptr) {
...
void dEnTorideKokoopa_c::notice2Vo() {
    if (mpParamDemo->mAnmNames[3] != nullptr) {
...
void dEnTorideKokoopa_c::shellAtkEffect() {
    mVec3_c pos(mShellJntPos.x, mShellJntPos.y, 5500.0f);
    if (mSpeed.x > 0.0f) {
        if (((EffectParam_t*)mUnkAF0)->mShellAtk[0] != nullptr) {
            mLevelEffects[1].createEffect(((EffectParam_t*)mUnkAF0)->mShellAtk[0], 0, &mShellJntPos, nullptr, nullptr);
        }
    } else if (mSpeed.x < 0.0f) {
        if (((EffectParam_t*)mUnkAF0)->mShellAtk[0] != nullptr) {
            mLevelEffects[1].createEffect(((EffectParam_t*)mUnkAF0)->mShellAtk[1], 0, &mShellJntPos, nullptr, nullptr);
        }
    }
    if (__fabs(mSpeed.x) < 0.5f * *(float*)((char*)this + 0x11c)) {
        if (*(float*)((char*)this + 0xf4) >= 0.0f) {
            if (((EffectParam_t*)mUnkAF0)->mShellAtkFast[0] != nullptr) {
                mEf::createEffect(((EffectParam_t*)mUnkAF0)->mShellAtkFast[0], 0, &mShellJntPos, nullptr, nullptr);
            }
        } else {
            if (((EffectParam_t*)mUnkAF0)->mShellAtkFast[1] != nullptr) {
                mEf::createEffect(((EffectParam_t*)mUnkAF0)->mShellAtkFast[1], 0, &mShellJntPos, nullptr, nullptr);
            }
        }
    }
    if (((EffectParam_t*)mUnkAF0)->mShellAtkLoop != nullptr) {
        mLevelEffects[2].createEffect(((EffectParam_t*)mUnkAF0)->mShellAtkLoop, 0, &mShellJntPos, nullptr, nullptr);
    }
}

void dEnTorideKokoopa_c::shellWallEffect() {
    static const s16 cs_ef_angle[2] = { (s16)0x8000, 0 };
    if (*(const char**)(mUnkAF0 + 0x44) != nullptr) {
        u8 dir = !(mSpeed.x >= 0.0f);
        mVec3_c wallOfs;
        mBc.getWallOfs(&wallOfs, 0);
        mVec3_c p1(mPos.x, mPos.y + wallOfs.y, mPos.z);
        mVec3_c p2(mPos.x + 24.0f * (float)l_EnMuki[mDirection], p1.y, mPos.z);
        float dist;
        dBc_c::checkWall(&p1, &p2, &dist, mLayer, 1, nullptr);
        p1.x = dist;
        p1.z = 5500.0f;
        mAng3_c angle(0, cs_ef_angle[dir], 0);
        mEf::createEffect(*(const char**)(mUnkAF0 + 0x44), 0, &p1, &angle, nullptr);
    }
}
...
void dEnTorideKokoopa_c::shellBumMarEffect() {
    const char *eff = *(const char**)mUnkAF0;
    if (eff != nullptr) {
        mVec3_c pos;
        pos.x = mShellJntPos.x;
        pos.y = mShellJntPos.y;
        pos.z = 5500.0f;
        mLevelEffects[0].createEffect(eff, 0, &mShellJntPos, nullptr, nullptr);
    }
}
...
void dEnTorideKokoopa_c::blitzMove(dActor_c *blitz) {
    if (blitz != nullptr) {
        calcKokoopaMdl();
        mBlitzPos = calcBlitzPos();
        blitz->mPos = mBlitzPos;
    }
}

void dEnTorideKokoopa_c::initializeState_Jump_St() {
    const char *anmName = mpParamJump->mAnmNames[0];
    if (anmName != nullptr) {
        ...
        mAnmChrKokoopa.setRate(mUnkACC);
    }
    mSpeed.x = 0.0f;
    mSpeed.y = 0.0f;
    mSpeed.z = 0.0f;
...
void dEnTorideKokoopa_c::executeState_Jump_St() {
    ...
    int angleOk = calcDirAngle(speed);
    if (mAnmChrKokoopa.isStop() && angleOk != 0) {
...
void dEnTorideKokoopa_c::initializeState_BigJump_St() {
    const char *anmName = mpParamJump->mAnmNames[2];
    if (anmName != nullptr) {
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(anmName);
        mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)1);
        mMdlKokoopa.setAnm(mAnmChrKokoopa, 0.0f);
...
void dEnTorideKokoopa_c::initializeState_LandOn() {
    const char *anmName = mpParamJump->mAnmNames[4];
    if (anmName != nullptr) {
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(anmName);
        mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)1);
        mMdlKokoopa.setAnm(mAnmChrKokoopa, 0.0f);
...
void dEnTorideKokoopa_c::initializeState_AttackReady() {
    const char *anmName = mpParamReady->mAnmNames[1];
    if (anmName != nullptr) {
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(anmName);
...
void dEnTorideKokoopa_c::initializeState_AttackBegin() {
    const char *anmName = mpParamAttack->mAnmNames[0];
    if (anmName != nullptr) {
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(anmName);
        mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)1);
        mMdlKokoopa.setAnm(mAnmChrKokoopa, 0.0f);
...
void dEnTorideKokoopa_c::executeState_AttackBegin() {
    ...
    lockonTurn();
    calcLookAngle();
    if (mAnmChrKokoopa.isStop() || mpParamAttack->mAnmNames[0] == nullptr) {
        changeState(StateID_AttackSearch);
    }
}

void dEnTorideKokoopa_c::initializeState_AttackSearch() {
    const char *anmName = mpParamAttack->mAnmNames[1];
    if (anmName != nullptr) {
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(anmName);
        mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)0);
        mMdlKokoopa.setAnm(mAnmChrKokoopa, 0.0f);
    ...
    const char *clrName = mpParamAttack->mClrName;
    if (clrName != nullptr) {
        nw4r::g3d::ResAnmClr clr = mResFile.GetResAnmClr(clrName);
        mAnmMatClr.setAnm(mMdlKokoopa, clr, 1, (m3d::playMode_e)1);
        mMdlKokoopa.setAnm(mAnmMatClr);
        mAnmMatClr.setFrame(0.0f, 1);
    }
    mCc.release();
    m_23b = 1;
...
void dEnTorideKokoopa_c::executeState_AttackSearch() {
    mMdlKokoopa.play();
    if (mpParamAttack->mClrName != nullptr) {
        mAnmMatClr.play(1);
    }
    calcSpeedY();
    posMove();
    ...
    case 1:
        if (isCreateBlitz()) {
            mUnk770 = createBlitz();
            if (mpBossLife->isTwoDamage() != 0 && mAtkCnt == 1) {
                mUnk768 = (s16)getAtkSearch2ndTime();
            } else {
                mUnk768 = (int)((float)(s16)getAtkSearchTime() / mUnkACC);
            }
            m_23b = 2;
        }
        break;
...
void dEnTorideKokoopa_c::initializeState_Attack() {
    const char *anmName = mpParamAttack->mAnmNames[2];
    if (anmName != nullptr) {
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(anmName);
        mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)1);
        mMdlKokoopa.setAnm(mAnmChrKokoopa, 0.0f);
    ...
    mCc.set(this, (sCcDatNewF*)&l_wand_cc);
    mCc.entry();
    m_23b = 1;
}
...
void dEnTorideKokoopa_c::initializeState_AttackEnd() {
    const char *anmName = mpParamAttack->mAnmNames[3];
    if (anmName != nullptr) {
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(anmName);
        mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)0);
        mMdlKokoopa.setAnm(mAnmChrKokoopa, 0.0f);
...
void dEnTorideKokoopa_c::initializeState_FireHit() {
    if (mpParamShell->mAnmNames[1] != nullptr) {
        const char *anmName = mpParamShell->mAnmNames[0];
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(anmName);
        mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)0);
        mMdlKokoopa.setAnm(mAnmChrKokoopa, 0.0f);
        mAnmChrKokoopa.setRate(1.0f);
...
void dEnTorideKokoopa_c::executeState_ShellAtk() {
    ...
    sLib::chaseAngle(&mAngle.z.mAngle, targetAngle, 0x80);
    shellatkSE();
    shellBumMarEffect();
    shellAtkEffect();
    if ((lastX < mUnkAC4 && mPos.x >= mUnkAC4) || (lastX > mUnkAC4 && mPos.x <= mUnkAC4)) {
        if (--mUnkAC8 > 0) {
            ...
            mSpeed.x = 0.0f;
            mDirection = getPl_LRflag(mPos);
            s32 dirAngle = defaultDirAngle();
            s8 muki = l_EnMuki[mDirection];
            mAngle.z = 0;
            mAngle.y = muki * dirAngle;
...
bool dEnTorideKokoopa_c::checkGetUp() const {
    return false;
}
...
void dEnTorideKokoopa_c::initializeState_DemoWait() {
    if (mpParamDemo != nullptr) {
        if (mpParamDemo->mAnmNames[0] != nullptr) {
            nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(mpParamDemo->mAnmNames[0]);
            mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)0);
            mMdlKokoopa.setAnm(mAnmChrKokoopa, 0.0f);
            mAnmChrKokoopa.setRate(1.0f);
        }
...
void dEnTorideKokoopa_c::initializeState_DemoAwake() {
    if (mpParamDemo->mAnmNames[1] != nullptr) {
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(mpParamDemo->mAnmNames[1]);
        mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)1);
        mMdlKokoopa.setAnm(mAnmChrKokoopa, 5.0f);
        mAnmChrKokoopa.setRate(1.0f);
        nw4r::g3d::ResAnmTexPat pat = mResFile.GetResAnmTexPat(mpParamDemo->mAnmNames[1]);
        mUnk6A8 = pat;
        mAnmTexPat.setAnm(mMdlKokoopa, pat, 0, (m3d::playMode_e)1);
        mMdlKokoopa.setAnm(mAnmTexPat, 0.0f);
...
void dEnTorideKokoopa_c::initializeState_DemoAwake_Wait() {
    if (mpParamDemo->mAnmNames[2] != nullptr) {
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(mpParamDemo->mAnmNames[2]);
        mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)0);
        mMdlKokoopa.setAnm(mAnmChrKokoopa, 0.0f);
        mAnmChrKokoopa.setRate(1.0f);
        nw4r::g3d::ResAnmTexPat pat = mResFile.GetResAnmTexPat(mpParamDemo->mAnmNames[2]);
        mUnk6A8 = pat;
        mAnmTexPat.setAnm(mMdlKokoopa, pat, 0, (m3d::playMode_e)0);
        mMdlKokoopa.setAnm(mAnmTexPat, 0.0f);
        mAnmTexPat.setRate(1.0f, 0);
    }
}
...
void dEnTorideKokoopa_c::initializeState_DemoIkaku() {
    if (mpParamDemo->mAnmNames[3] != nullptr) {
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(mpParamDemo->mAnmNames[3]);
        mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)1);
        mMdlKokoopa.setAnm(mAnmChrKokoopa, 0.0f);
        mAnmChrKokoopa.setRate(1.0f);
        nw4r::g3d::ResAnmTexPat pat = mResFile.GetResAnmTexPat(mpParamDemo->mAnmNames[3]);
        mUnk6A8 = pat;
        mAnmTexPat.setAnm(mMdlKokoopa, pat, 0, (m3d::playMode_e)1);
        mMdlKokoopa.setAnm(mAnmTexPat, 0.0f);
        mAnmTexPat.setRate(1.0f, 0);
...
void dEnTorideKokoopa_c::initializeState_DemoIkaku_Wait() {
    if (mpParamDemo->mAnmNames[4] != nullptr) {
        nw4r::g3d::ResAnmChr anm = mResFile.GetResAnmChr(mpParamDemo->mAnmNames[4]);
        mAnmChrKokoopa.setAnm(mMdlKokoopa, anm, (m3d::playMode_e)0);
        mMdlKokoopa.setAnm(mAnmChrKokoopa, 0.0f);
        mAnmChrKokoopa.setRate(1.0f);
        nw4r::g3d::ResAnmTexPat pat = mResFile.GetResAnmTexPat(mpParamDemo->mAnmNames[4]);
        mUnk6A8 = pat;
        mAnmTexPat.setAnm(mMdlKokoopa, pat, 0, (m3d::playMode_e)0);
        mMdlKokoopa.setAnm(mAnmTexPat, 0.0f);
        mAnmTexPat.setRate(1.0f, 0);
