// d_bg_ctr.cpp -- FIRST DRAFT (scratch/round24/d_bg_ctr)
//
...
static void fn_8007FFA0(dBg_ctr_c *ctr, dActor_c *actor, mVec3_c *vec, int mode) {
    ...
    if (ctr->m_c2 != 0) {
        ...
        f32 cos = nw4r::math::CosFIdx((f32)angle * (1.0f / 256.0f));
        stack_x += sqrtVal * cos - src->x;

        f32 sin = nw4r::math::SinFIdx((f32)angle * (1.0f / 256.0f));
...
void dBg_ctr_c::calc() {
    ...
    if (mMode == 1) {
        ...
        f32 cos = nw4r::math::CosFIdx((f32)rot * (1.0f / 256.0f));
        f32 sin = nw4r::math::SinFIdx((f32)rot * (1.0f / 256.0f));
...
void dBg_ctr_c::addDokanMoveDiff(mVec3_c *out) {
    ...
    out->x = dScStage_c::getLoopPosX(mPos.x + len * nw4r::math::CosFIdx((f32)angle * (1.0f / 256.0f)));
    out->y = mPos.y + len * nw4r::math::SinFIdx((f32)angle * (1.0f / 256.0f));
}
...
bool dBg_ctr_c::fn_80080900(mVec3_c *segment, short *angleOut, int flags) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (nw4r::math::DistSqSegment3ToSegment3(
                reinterpret_cast<const nw4r::math::SEGMENT3 *>(segment),
