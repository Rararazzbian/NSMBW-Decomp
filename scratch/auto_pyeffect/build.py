"""Build/diff driver for the dPyEffectMng_c unit (Linux container).

    python3 scratch/auto_pyeffect/build.py
"""
import os
import sys

ROOT = '/opt/NSMBW-Decomp'
BASE = os.path.join(ROOT, 'scratch', 'auto_pyeffect')
...
TARGET = os.path.join(ROOT, 'tools', 'auto_decomp', 'work',
                      'dol_bases_d_player_effect_manager', 'target.txt')

FNS = [
    '__ct__14dPyEffectMng_cFv',
    '__dt__14dPyEffectMng_cFv',
    'fn_800d2de0__14dPyEffectMng_cFfiR7mVec3_cUc',
    'update__14dPyEffectMng_cFv',
]

INC = (os.path.join(BASE, 'shadow'),)


def build(source='d_player_effect_manager.cpp', label=None):
...
if __name__ == '__main__':
    src = sys.argv[1] if len(sys.argv) > 1 else 'd_player_effect_manager.cpp'
    txt = build(src)
    if txt:
        rep, bad = report(txt)
        print(rep)
