"""Build/diff driver for the dPosShake_c unit (Linux container).

    python3 scratch/auto_posshake/build.py
    python3 scratch/auto_posshake/build.py swX.cpp   # build a variant
"""
import os
import sys

ROOT = '/opt/NSMBW-Decomp'
BASE = os.path.join(ROOT, 'scratch', 'auto_posshake')
...
def build(source='d_pos_shake.cpp', label=None):
    ...
    inc = (os.environ.get('SHADOW') and (os.path.join(BASE, os.environ['SHADOW']),)) or ((os.path.join(BASE, 'shadow'),) if os.path.isdir(os.path.join(BASE, 'shadow')) else ())
...
def report(txt):
    ...
    for name in FNS:
        ...
        out.append('%-30s %s (%dw/%dw)' % (name, 'MATCH' if matched else head, t, n))
    return '\n'.join(out), total


if __name__ == '__main__':
    src = sys.argv[1] if len(sys.argv) > 1 else 'd_pos_shake.cpp'
    txt = build(src)
    if txt:
        rep, bad = report(txt)
        print(rep)
