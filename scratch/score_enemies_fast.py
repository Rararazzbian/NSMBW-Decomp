...
# Disassemble all d_enemiesNP split objects if not done
...
scored_tus = []
for tu in tus:
    if tu['code'] < 10000 and tu['fns'] >= 4:
        se, ss, fn_cnt, b_cnt = score_tu(tu)
        scored_tus.append((se, ss, tu, fn_cnt, b_cnt))
        p_names = ', '.join(tu['profs']) if tu['profs'] else 'NO_PROFILE'
        print(f"TU {tu['tu_idx']:2d} ({p_names[:25]}): code={b_cnt}B, exact={se*100:.2f}%, shape={ss*100:.2f}%", flush=True)
...
print('=== Top 20 Candidates by Sibling Score ===')
for idx, (se, ss, tu, fn_cnt, b_cnt) in enumerate(scored_tus[:20], 1):
    p_names = ', '.join(tu['profs']) if tu['profs'] else 'NO_PROFILE'
    print(f"{idx:2d}. TU {tu['tu_idx']:2d} ({p_names}): {b_cnt} B ({fn_cnt} fns) | Exact: {se*100:.2f}%, Shape: {ss*100:.2f}%")
