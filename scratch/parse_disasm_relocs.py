...
def parse_relocs_from_disasm(disasm_files, tu_name, tu_ranges):
    print(f"\n=======================================================")
    ...
    # 2. Extract referenced symbols from disassembly text files
    referenced_syms = set()
    defined_syms = set()
    ...
    for fpath in disasm_files:
        with open(fpath, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                ...
                if m_fn:
                    defined_syms.add(m_fn.group(1).strip())
                
                # Instruction line: /* ... */ instr operands
                # Look for symbols in operands: bl symbol, or symbol@ha, symbol@l, symbol@sda21, symbol@sdarx
                # Also handle quoted symbols like "@14502_8042E010"@sda21
                if '/*' in line and '*/' in line:
                    instr_part = line.split('*/', 1)[1].strip()
                    # Find all relocations: word@qual or bl target
                    # match bl <sym>
    ...
    print(f"Defined in split asm: {len(defined_syms)}")
    print(f"Referenced in split asm: {len(referenced_syms)}")

    # Filter out internal symbols
    external_syms = set()
    for sym in referenced_syms:
        # Check if defined in TU
        if sym in defined_syms:
            continue
        # Check if internal to TU address ranges
        if sym in all_symbols:
            ...
            if is_internal:
                continue
        # Also skip runtime save/restore helpers if standard or check them
    ...
    for sym in sorted(external_syms):
        ...
        if sym in all_symbols:
            ...
            # e.g. @14502_8042E010 or lbl_8042B688
            try:
...
parse_relocs_from_disasm(pad_disasms, "m_pad.cpp", pad_ranges)
...
coin_ranges = {
    ...
    '.rodata': (0x802EE750, 0x802EE810),
    '.data': (0x80303078, 0x80303368),
    '.bss': (0x803530E8, 0x80353120),
    '.sdata2': (0x8042B630, 0x8042B638)
}
parse_relocs_from_disasm(coin_disasms, "d_a_en_coin_main.cpp", coin_ranges)
