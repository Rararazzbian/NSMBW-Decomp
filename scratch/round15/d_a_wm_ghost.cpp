...
const float sGhostUnusedFloats[] __attribute__((used)) = { 1.3f, 180.0f };
...
int daWmGhost_c::create() {
    ...
    initState();
    calcModel();
    mClipSphere.set(mPos, sGhostUnusedFloats[1]);
    return SUCCEEDED;
}
...
void daWmGhost_c::createModel() {
    ...
    resMdl = resFile.GetResMdl("cobGhost");

    mModel.create(resMdl, &mAllocator, nw4r::g3d::ScnMdl::BUFFER_RESMATMISC, 1, nullptr);

    for (i = 0; i < ANIM_COUNT; i++) {
        mChrAnim[i].create(resMdl, resFile.GetResAnmChr(resAnmNames[i]), &mAllocator, nullptr);
