...
src_path = os.path.join(ROOT, "scratch", "gemini_round4", "d_nand_thread_full.cpp")
obj_path = os.path.join(ROOT, "scratch", "gemini_round4", "d_nand_thread_full.o")
txt_path = os.path.join(ROOT, "scratch", "gemini_round4", "d_nand_thread_full.txt")

with open(src_path, "w", encoding="ascii") as f:
    f.write(full_scaffold_cpp)

args = [MWCC] + CFLAGS + [src_path, "-o", obj_path]
for inc in INCLUDES: args += ["-i", inc]
p = subprocess.run(args, cwd=ROOT, capture_output=True, text=True)
print("Scaffold Compile:", "OK" if p.returncode == 0 else "FAIL")
if p.returncode != 0:
    print(p.stdout, p.stderr)
else:
    subprocess.run([DTK, "elf", "disasm", obj_path, txt_path], cwd=ROOT)
    print("Disassembled scaffold to", txt_path)
