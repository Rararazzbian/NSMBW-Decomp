...
public:
    ...
    /// @brief Gets a copy of the controller's DPD raw position.
    /// @note Declared to return Vector2f (non-POD): MWCC returns it through the
    /// hidden sret pointer, matching the retail call shape.
    Vector2f getDpdRawPos() const;

    /// @brief Gets the controller's DPD distance.
    float getDpdDistance() const;
