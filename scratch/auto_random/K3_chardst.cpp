...
struct Collector {
    static void collectOne(char *dst, int i) {
        ...
        *reinterpret_cast<float *>(dst + 0) = b.x;
        *reinterpret_cast<float *>(dst + 4) = b.y;
        *reinterpret_cast<float *>(dst + 8) = b.z;
        *reinterpret_cast<float *>(dst + 12) = raw.x;
        *reinterpret_cast<float *>(dst + 16) = raw.y;
...
u32 dRandom_c::calcMachineRandom() {
    ...
    *(s64 *)buf = OSGetTime();
    ...
    int i = 0;
    do {
        Collector::collectOne(&buf[0x20] + i * 0x18, i);
    } while (++i <= 3);
