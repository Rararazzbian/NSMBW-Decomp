...
u32 dRandom_c::calcMachineRandom() {
    ...
    *(s64 *)buf = OSGetTime();
    ...
    do {
        ...
        float *dst = None;
        reinterpret_cast<float *>(buf + 0x20)[i * 6 + 0] = b.x;
        reinterpret_cast<float *>(buf + 0x20)[i * 6 + 1] = b.y;
        reinterpret_cast<float *>(buf + 0x20)[i * 6 + 2] = b.z;
        mVec2_c raw;
        reinterpret_cast<float *>(buf + 0x20)[i * 6 + 3] = rp.x;
        reinterpret_cast<float *>(buf + 0x20)[i * 6 + 4] = rp.y;
        (void)raw;
        reinterpret_cast<float *>(buf + 0x20)[i * 6 + 5] = mPad::g_core[i]->getDpdDistance();
    } while (++i <= 3);
