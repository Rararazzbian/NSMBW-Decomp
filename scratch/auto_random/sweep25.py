"""Sweep 25: by-value inline helper producing the mVec3_c copies.

Reference-copy form always yields fst24/readback-first-slot. A static-inline
helper RETURNING mVec3_c by value gives sret semantics (helper writes the
return slot directly), which may pin a-first/b-second ordering.
"""
import os
import re
import sys
...
HELPER_CC = '''static inline mVec3_c getCorePos(const EGG::CoreController *cc) {
    return *reinterpret_cast<const mVec3_c *>(
        reinterpret_cast<const char *>(cc) + 0x24);
}

'''

HELPER_I = '''static inline mVec3_c getCorePos(int i) {
    return *reinterpret_cast<const mVec3_c *>(
...
def mk(helper, abform):
    return HEAD + helper + '''u32 dRandom_c::calcMachineRandom() {
    char buf[0x80];
...
AB_REF = '''        const mVec3_c &r =
            *reinterpret_cast<const mVec3_c *>(
                reinterpret_cast<const char *>(mPad::g_core[i]) + 0x24);
        mVec3_c a(r);
        mVec3_c b(a);'''

AB_HCC = '''        mVec3_c a(getCorePos(mPad::g_core[i]));
        mVec3_c b(a);'''

AB_HI = '''        mVec3_c a(getCorePos(i));
        mVec3_c b(a);'''

V = {
    'h1_helper_cc': mk(HELPER_CC, AB_HCC),
    'h2_helper_i': mk(HELPER_I, AB_HI),
    'h0_ref_baseline': mk('', AB_REF),
}
