"""Build/diff driver for the dRotShake_c unit (Linux container).

    python3 scratch/auto_rotshake/build.py            # build draft -> draft.txt
    python3 scratch/auto_rotshake/build.py swX_name   # build swX_name.cpp instead
"""
import os
import sys

ROOT = '/opt/NSMBW-Decomp'
BASE = os.path.join(ROOT, 'scratch', 'auto_rotshake')
...
FNS = [
    'init__11dRotShake_cFssssssss',
    'move__11dRotShake_cFv',
]
...
if __name__ == '__main__':
    src = sys.argv[1] if len(sys.argv) > 1 else 'd_rot_shake.cpp'
    txt = build(src)
    if txt:
        rep, bad = report(txt)
        print(rep)
