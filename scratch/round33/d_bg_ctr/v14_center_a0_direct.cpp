...
void dBg_ctr_c::calc() {
    ...
    if (mMode == 1) {
        ...
        m_a0.x = (centerPos.x + mScratch[2].x) * 0.5f;
        m_a0.y = (centerPos.y + mScratch[2].y) * 0.5f;
    }
...
void dBg_ctr_c::addDokanMoveDiff(mVec3_c *out) {
    ...
    out->y = mPos.y + len * nw4r::math::SinIdx(angle);
}
