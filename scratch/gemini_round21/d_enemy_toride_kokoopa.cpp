...
void dEnTorideKokoopa_c::initializeState_Jump() {
    ...
    mSpeed.y = speed.y;
    mSpeed.x = ((f32)l_EnMuki[mDirection] * calcJumpRate()) * speed.x;
    jumpEffect();
    jumpSE();
}
...
void dEnTorideKokoopa_c::executeState_AttackSearch() {
    ...
    case 1:
        ...
        break;
    case 2: {
        dActor_c *act = nullptr;
        if (mUnk770 != 0) {
            act = (dActor_c*)fManager_c::searchBaseByID((fBaseID_e)mUnk770);
        }
        blitzMove(act);
        if (--mUnk768 <= 0) {
            if (lockon) {
