...
bool KokoopaSpFumiCheck_c::operate(int &out, dEn_c *en, FumiCcInfo_c &info) {
    ...
    if (((((FumiPlayerView *) info.mCc2->getOwner())->mFlagA | ((FumiPlayerView *) info.mCc2->getOwner())->mFlagB) != 0) && ((FumiPlayerView *) info.mCc2->getOwner())->mEC > 0.0f) {
        out = 0;
        return true;
    }

    if (!((dBc_c *) ((u8 *) ((FumiPlayerView *) info.mCc2->getOwner()) + 0x1EC))->isFoot()) {
        if (ev->mEC > 0.0f) {
            if (((FumiPlayerView *) info.mCc2->getOwner())->mMode == 3) {
                if (((FumiPlayerView *) info.mCc2->getOwner())->mB0 >= ev->mB0 + 4.0f) {
                    const s8 *tbl = ((FumiPlayerView *) info.mCc2->getOwner())->getDamageTable();
                    s8 idx = tbl[0];
                ...
                if (((FumiPlayerView *) info.mCc2->getOwner())->mB0 >= ev->mB0 + 10.0f) {
                    const s8 *tbl = ((FumiPlayerView *) info.mCc2->getOwner())->getDamageTable();
                    s8 idx = tbl[0];
                    ev->mFumiVals[idx] = 0x18;
                    out = 1;
