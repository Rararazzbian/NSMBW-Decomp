...
float FumiCcInfo_c::getFumiRev() {
    dCc_c *cc2 = mCc2;
    dActor_c *owner = cc2->getOwner();
    u8 st = *((u8 *)owner + 0x38C);
    if (st == 1) {
        if (*((u8 *)cc2 + 0x2D) != 0xB) {
            return *(float *)((u8 *)mCc1 + 0x70);
        }
        return *(float *)((u8 *)mCc1 + 0x74);
    }
    if (st == 2) {
        return *(float *)((u8 *)mCc1 + 0x78);
    }
    return 0.0f;
}

bool MugenComboFumiCheck_c::operate(int &out, dEn_c *, FumiCcInfo_c &info) {
    out = 0;
    dActor_c *owner = info.mCc2->getOwner();
    if ((*(u32 *)((u8 *)owner + 0x10D4) & 1) == 0) {
        if (*(float *)((u8 *)owner + 0xEC) < 0.0f) {
            out = 1;
        }
    }
    return true;
