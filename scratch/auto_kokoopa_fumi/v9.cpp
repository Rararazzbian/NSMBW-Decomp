...
bool KokoopaSpFumiCheck_c::operate(int &out, dEn_c *en, FumiCcInfo_c &info) {
    out = 0;
    dCc_c *cc = info.mCc2;
    FumiPlayerView *pl = (FumiPlayerView *) cc->getOwner();
    ...
    if (((dBc_c *) ((u8 *) pl + 0x1EC))->isFoot() == 0 &&
        ((FumiEnemyView *) en)->mEC > l_poolZero_8042C768) {
        if (pl->mMode == 3) {
            if (pl->mB0 >= l_poolFour_8042C76C + ((FumiEnemyView *) en)->mB0) {
            ...
            if (pl->mB0 >= l_poolTen_8042C770 + ((FumiEnemyView *) en)->mB0) {
                const s8 *tbl = pl->getDamageTable();
                s8 idx = tbl[0];
                ((FumiEnemyView *) en)->mFumiVals[idx] = 0x18;
                out = 1;
