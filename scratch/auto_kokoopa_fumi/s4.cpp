...
bool KokoopaSpFumiCheck_c::operate(int &out, dEn_c *en, FumiCcInfo_c &info) {
    ...
    if (((dBc_c *) ((u8 *) pl + 0x1EC))->isFoot() == 0 &&
        ev->mEC > l_poolZero_8042C768) {
        if (pl->mMode == 3) {
            if (pl->mB0 >= l_poolFour_8042C76C + ev->mB0) {
            ...
            if (pl->mB0 >= l_poolTen_8042C770 + ev->mB0) {
                const s8 *tbl = pl->getDamageTable();
                s8 idx = tbl[0];
                ev->mFumiVals[idx] = 0x18;
                out = 1;
