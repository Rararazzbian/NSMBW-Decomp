...
bool KokoopaSpFumiCheck_c::operate(int &out, dEn_c *en, FumiCcInfo_c &info) {
    out = 0;
    dActor_c *own = info.mCc2->getOwner();

    if (((((FumiPlayerView *) own)->mFlagA | ((FumiPlayerView *) own)->mFlagB) != 0) &&
        ((FumiPlayerView *) own)->mEC > l_poolZero_8042C768) {
        out = 0;
        return true;
    }

    if (((dBc_c *) ((u8 *) own + 0x1EC))->isFoot() == 0 &&
        ((FumiEnemyView *) en)->mEC > l_poolZero_8042C768) {
        if (((FumiPlayerView *) own)->mMode == 3) {
            if (((FumiPlayerView *) own)->mB0 >= l_poolFour_8042C76C + ((FumiEnemyView *) en)->mB0) {
                const s8 *tbl = ((FumiPlayerView *) own)->getDamageTable();
                s8 idx = tbl[0];
                ((FumiEnemyView *) en)->mFumiVals[idx] = 0x18;
                out = 1;
                return true;
            }
        } else {
            if (((FumiPlayerView *) own)->mB0 >= l_poolTen_8042C770 + ((FumiEnemyView *) en)->mB0) {
                const s8 *tbl = ((FumiPlayerView *) own)->getDamageTable();
                s8 idx = tbl[0];
                ((FumiEnemyView *) en)->mFumiVals[idx] = 0x18;
                out = 1;
