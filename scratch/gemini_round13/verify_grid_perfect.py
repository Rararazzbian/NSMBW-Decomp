...
from compare_grid_dummys import codeA

test_grid_cpp(codeA)

target = functions(os.path.join(ROOT, 'bin', 'dtkspl', 'd_basesNP', 'obj', 'auto_fn_2_164380_text.o.dis.txt'), with_addr=True)
draft = functions(os.path.join(ROOT, 'scratch', 'gemini_round13', 'd_a_wm_grid_compiled.txt'))

t_name, t_ins = [(name, ins) for addr, name, ins in target if addr == 0x164380][0]
