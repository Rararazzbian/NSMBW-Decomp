...
print(f"Parsed {len(symbols)} symbols from wiimj2d_symbols.txt")

# Read DOL header
# DOL header format:
# 7 text section offsets (u32*7)
# 11 data section offsets (u32*11)
# 7 text section addrs (u32*7)
# 11 data section addrs (u32*11)
# 7 text section sizes (u32*7)
# 11 data section sizes (u32*11)
# bss addr (u32)
# bss size (u32)
# entry point (u32)
...
print(f".ctors start at VA {hex(ctors_base)}, size {hex(ctors_size)} ({ctors_size//4} entries)")

addr_to_sym = {s['addr']: s for s in symbols}

for idx in range(ctors_size // 4):
