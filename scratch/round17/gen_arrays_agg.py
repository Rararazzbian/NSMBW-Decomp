...
# d_bg_actor_mng.cpp in AGGREGATE { ... } form (matching the target's .data:
# int fields folded, mVec3_c/mVec2_c ctor-call initializers that zero the
# floats in .data and are written at runtime by __sinit_).
import struct
import os
...
for name, base, count in ARRAYS:
    out.append('dBgActorManager_c::BgObjName_t %s[%d] = {' % (name, count))
    for i in range(count):
        ...
        out.append('    {0x%X, 0x%X, 0x%X, mVec3_c(%s), mVec2_c(%s), 0x%X},' %
                   (unit, nm, flag, offs, sizs, param))
        global_idx += 1
    out.append('};')
    out.append('')

out.append('dBgActorManager_c::BgObjName_t *l_rail_list[5] = {')
out.append('    l_object_name, l_Pa3_rail, l_Pa3_rail, l_Pa3_daishizen,')
out.append('    l_Pa3_MG_house_ami_rail,')
out.append('};')

with open(os.path.join(r'C:\Users\Razz\Documents\Projects\NSMBW-Decomp\scratch\round17',
                       'arrays_aggregate.txt'), 'w') as fh:
    fh.write('\n'.join(out))
print('\n'.join(out[:16]))
print('...')
