...
once and produced dozens of spurious diffs.
    ...
    python tools/unit_verify.py <draft.cpp> [--neg] [--lo 0xADDR --hi 0xADDR
                                                --unit dol_bases_<name>]
...
# Unit under test. Defaults are the last unit this was used on; override with
# --lo/--hi/--unit so the next unit does not have to fork the script.
LO = int(os.environ.get('UV_LO', '0x8005B3A0'), 16)
HI = int(os.environ.get('UV_HI', '0x8005D7E0'), 16)
UNIT = os.environ.get('UV_UNIT', 'dol_bases_d_a_player_demo_manager')
for _i, _a in enumerate(sys.argv):
    if _a == '--lo':
        LO = int(sys.argv[_i + 1], 16)
    elif _a == '--hi':
        HI = int(sys.argv[_i + 1], 16)
    elif _a == '--unit':
        UNIT = sys.argv[_i + 1]
TARGET = os.path.join(ROOT, 'tools', 'auto_decomp', 'work', UNIT, 'target.txt')
