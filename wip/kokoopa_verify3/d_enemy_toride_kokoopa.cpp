...
void dEnTorideKokoopa_c::initializeState_Jump() {
    ...
    int flag = (!mpBossLife->isOneDamage() && !mpBossLife->isTwoDamage()) ? 1 : 0;
    mVec2_c speed;
    if (flag != 0) {
...
void dEnTorideKokoopa_c::initializeState_ShellAtk_St() {
    ...
    if (oldState->isEqual(StateID_FireHit) || oldState->isEqual(StateID_SlideHit) || oldState->isEqual(StateID_StarHit)) {
        mAccelY = -0.275f;
        if (checkDownJump()) {
...
void dEnTorideKokoopa_c::executeState_ShellAtk_St() {
    ...
    const sStateIDIf_c *oldState = mStateMgr.getOldStateID();
    if (oldState->isEqual(StateID_FireHit) || oldState->isEqual(StateID_SlideHit)) {
    ...
    if (mUnk794 == 2) {
        ...
        shellChangeEffect();
        shellLandonEffect();
    } else if (mStateMgr.getOldStateID()->isEqual(StateID_FireHit)) {
        hitFireDamageEffect();
    }
    mAngle.z = 0;
    if (mBc.checkFootEnm()) {
        shelllandonSE();
        downLandOnEffect(0.0f);
        if (--mUnkAA0 > 0) {
...
void dEnTorideKokoopa_c::executeState_AttackSearch() {
    ...
    case 1:
        if (isCreateBlitz()) {
            mUnk770 = createBlitz();
            if (mpBossLife->isDmgSection() && mAtkCnt == 1) {
        ...
        break;
    case 2:
        blitzMove((mUnk770 == 0) ? (dActor_c*)nullptr : (dActor_c*)fManager_c::searchBaseByID((fBaseID_e)mUnk770));
        if (--mUnk768 <= 0) {
            if (lockon) {
