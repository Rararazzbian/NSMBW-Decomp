...
/// @unofficial SHADOW COPY (proposal, not landed) of include/game/bases/d_info.hpp.
/// The only change from the real header is splitting `pad3[0x8]` (0x58-0x60)
/// into `pad3[0x4]` (0x58-0x5c) + a new named `mStartPointKinokoHouseKind`
/// (0x5c-0x60). daWmKinokoBase_c::processCutsceneCommand's cutscene-command-0x3c
/// branch does `stw r0, 0x5c(r4)` on `dInfo_c::m_instance` with r0 the
/// zero-extended low byte of `dWmLib::getStartPointKinokoHouseKindNum()`'s
/// result -- a real 4-byte field at +0x5c, not padding. This is the same kind
/// of pad-that-was-actually-a-field fix the real header's own m_64/m_68
/// comment already documents at +0x64/+0x68; the same trick applies one
/// field earlier, at +0x5c, discovered while authoring this unit.
class dInfo_c {
public:
    /// @unofficial
    enum GameMode_e {
    ...
    int m_54;
    u8 pad3[0x4];
    /// @unofficial [+0x5c] PROPOSED new field, was `pad3`'s tail 4 bytes.
    /// Set from `dWmLib::getStartPointKinokoHouseKindNum()` by
    /// daWmKinokoBase_c::processCutsceneCommand. Declared `int` because the
    /// target stores a `stw` (zero-extended from a byte), not a `stb`.
