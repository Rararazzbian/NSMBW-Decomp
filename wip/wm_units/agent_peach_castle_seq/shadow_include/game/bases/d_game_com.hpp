...
/// @unofficial SHADOW COPY, proposed addition to the real include/game/bases/d_game_com.hpp:
/// adds `ModelPlayMenuStart()`, mangled `ModelPlayMenuStart__8dGameComFv`, called from
/// daPeachCastleSequenceMgrObj_c::executeState_Wait (fn_2_1209A0) at 0x1209FC. Return value
/// is not used by the caller (result register untouched afterward), so `void` is taken as the
/// minimal-assumption return type. Everything else in this file is an unmodified copy of the
/// real header, needed only so the added declaration sits in the same namespace.
///
...
namespace dGameCom {
    ...
    /// @unofficial Proposed this round. Starts playback of some "menu" animation/model; called
    /// unconditionally once a Peach's Castle sequence countdown expires. `Fv` mangling confirms
    /// no parameters.
