...
/// @unofficial SHADOW COPY, proposed addition to the real include/game/bases/d_s_stage.hpp:
/// adds `m_isOtehonReturn`, mangled `m_isOtehonReturn__10dScStage_c`, read from
/// daPeachCastleSequenceMgrObj_c::initializeState_Wait (fn_2_120970) at 0x120980 as a plain
/// `lbz`+`cmpwi`+`beqlr` (byte load, not the neg/or/srwi bool-canonicalisation tail), so it is
/// only ever READ here, never written -- `bool` is taken as the natural type for a flag that is
/// tested directly against zero. Its exact position relative to the other dScStage_c statics is
/// NOT re-verified in this pass (no .sbss address evidence gathered for it); it is appended after
/// the existing statics rather than interleaved, so as not to assert an unverified order.
/// Everything else in this file is an unmodified copy of the real header.
...
public:
    ...
    /// @unofficial Proposed this round -- see file header comment.
