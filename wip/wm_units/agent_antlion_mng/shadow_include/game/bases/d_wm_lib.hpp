...
/// @unofficial SHADOW COPY, proposed addition to the real include/game/bases/d_wm_lib.hpp: adds
/// `setEnemyRevivalCount(int, int, int)`, mangled `setEnemyRevivalCount__6dWmLibFiii`, called from
/// daWmAntlionMng_c::primeRevivalCount (fn_2_15C0D0). Everything else in this file is an unmodified
/// copy of the real header, needed only so the added declaration sits in the same namespace.
namespace dWmLib {
    enum Direction3D_e {
        DIR3D_UP,
        DIR3D_DOWN,
    ...
    int setEnemyRevivalCount(int, int, int);
