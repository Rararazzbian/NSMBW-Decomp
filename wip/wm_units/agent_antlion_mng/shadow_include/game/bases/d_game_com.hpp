...
/// adds `getRandom(unsigned long)`, mangled `getRandom__8dGameComFUl`, called from
/// daWmAntlionMng_c::pickRevivedIndices (fn_2_15BA70). Everything else in this file is an
/// unmodified copy of the real header, needed only so the added declaration sits in the same
/// namespace.
///
...
namespace dGameCom {
    ...
    /// @unofficial Proposed this round. `getRandom__8dGameComFUl` is a straight tail call
    /// (`mr r4,r3; li r3,m_rnd@sda21; b ranqd1__9cM_rand_cFUi`, confirmed against
    /// `scratch/auto_03_800B2C28_text.o.txt`) into `cM_rand_c::ranqd1(unsigned int)`, so its own
    /// return register is ranqd1's untouched -- return type taken as `u32` to match.
