...
namespace dGameCom {
    ...
    /// @unofficial SHADOW ADDITIONS (agent_gun_battery). Called from
    /// `daMiniGameGunBatteryMgrObj_c::executeState_ShowRule`/
    /// `executeState_ShowResult` (`fn_2_F8F70`/`fn_2_F9320`). Names are REAL,
    /// pulled directly from `bin/dtk/wiimj2d_symbols.txt` (the DOL's own
    /// symbol table), not guessed:
    ///   MiniGameCannonTitle__8dGameComFv               = .text:0x800B4940
    ///   MiniGameCannonStart__8dGameComFv                = .text:0x800B4970
    ///   MiniGameCannonResult__8dGameComFv               = .text:0x800B49A0
    ///   MiniGameCannonEndCheck__8dGameComFv             = .text:0x800B49D0
    ///   MiniGameCannonTitleCloseRequest__8dGameComFv    = .text:0x800B4A00
    ///   MiniGameCannonOperateCloseRequest__8dGameComFv  = .text:0x800B4A30
    ///   MiniGameCannonResultCloseRequest__8dGameComFv   = .text:0x800B4A60
    /// Signatures pinned by the call sites: all no-arg, void except
    /// `MiniGameCannonEndCheck` which returns a value compared against 0.
