...
/// @unofficial SHADOW COPY, proposed addition to the real include/game/bases/d_wm_lib.hpp:
/// adds `MakePointNameFromCourseNo(int, int, char*)`, mangled
/// `MakePointNameFromCourseNo__6dWmLibFiiPc`, called from daWmKinoBalloon_c (fn_2_16A7C0).
...
namespace dWmLib {
    ...
    /// @unofficial Proposed this round. `MakePointNameFromCourseNo__6dWmLibFiiPc`, called
    /// from daWmKinoBalloon_c (fn_2_16A7C0) as MakePointNameFromCourseNo(world, course, dst).
    /// Return type `int` -- the call site reuses r3 (this function's own return value) directly
    /// as `GetPos`'s int node-index argument, so it cannot be `void`.
    int MakePointNameFromCourseNo(int world, int course, char *dst);
