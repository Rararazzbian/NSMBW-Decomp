...
SRC = os.path.join('wip', 'author_core', 'd_line_mng.cpp')
OBJ = os.path.join('wip', 'author_core', 'd_line_mng.o')
TXT = os.path.join('wip', 'author_core', 'd_line_mng.txt')
TARGET = os.path.join('wip', 'line_mng_shared', 'target.txt')
SHADOW = os.path.join('wip', 'line_mng_shared', 'shadow_include')
LOCAL_SHADOW = os.path.join('wip', 'author_core', 'local_shadow')

def build():
    ok, log = H.compile_draft(SRC, OBJ, extra_inc=[LOCAL_SHADOW, SHADOW])
    if not ok:
