...
bool dLineMng_c::line_cross_chk2(f32 p1, const mVec2_c &p3, mVec2_c p4, mVec2_c p5, f32 &out) {
    ...
    // zero_ref() on the FIRST guard only (lever 11 route 3, applied to fcmpu).
    // This is what unblocked the function after ~35 recorded failed variants.
    // A syntactic `0.0f` is hoisted into the compare's first operand slot by
    // MWCC's front-end canonicaliser, which makes the constant the FIRST leaf
    // evaluated -- so its load is scheduled ahead of everything, right after
    // the `fmr f31, f1` at word 7, and it takes f1. That pins f1 for the whole
    // prologue and forces the four p4/p5 field subtractions into f2/f0 instead
    // of retail's f0/f1. Routing the constant through zero_ref() makes it a
    // plain parameter at parse time, so the canonicaliser never sees a
    // literal, the value leaves evaluate first, and the 0.0f load drops into
    // the slot freed when p4.x's raw value dies mid-subtraction -- exactly
    // where retail has it (word 19). Closes the whole prologue+guard block:
    // 27 differing -> 6, with words 0..74 byte-exact.
    // NOTE: extending zero_ref() to the SECOND guard's < / >= compares is
    // measurably WORSE (6 -> 7); only the `!=` pair wants it.
    if (p4.x != zero_ref(0.0f) && p5.x != zero_ref(0.0f)) {
    ...
    if (line_cross_slope_check(p4, p5, slope, intercept)) {
        ...
        // The else arm needs the OPPOSITE treatment to the guard, and it needs
        // BOTH halves of it -- this is a lever-13 + lever-11-route-3
        // composition, and neither half alone moves the count (each is 6).
        //
        //  - `f32 x5` / `f32 y5` (lever 13, read-side def-point): retail holds
        //    p5.x and then p5.y in f1 and the shared 0.0f in f0. Bare member
        //    reads are un-def-pointed leaves, so they number BELOW the
        //    constant and the two registers come out swapped. Naming both
        //    reads lifts them above it. Naming only one is not enough: the two
        //    compares share the constant's register, so both reads have to
        //    outrank it. 6 -> 1.
        //  - `zero_ref` on the remaining `==` (route 3): with the registers
        //    now right, the last diff was pure operand SLOT --
        //    `fcmpu f0, f1` against retail's `fcmpu f1, f0`. 1 -> 0.
        f32 x5 = p5.x;
        if (x5 == zero_ref(0.0f)) {
            f32 y5 = p5.y;
            if (y5 >= 0.0f && y5 < p1) {
