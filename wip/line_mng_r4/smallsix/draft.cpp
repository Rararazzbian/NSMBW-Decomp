...
// same bodies, same evidence.
//
// CONTRADICTION, per AGENT_CONTEXT.md rule 4: this used to be declared
// `static` on the claim that "dtk shows no symbol name for them at all in
// the target -- consistent with the original binary's own stripped internal
// statics." That is measurably false: wip/line_mng_shared/target.txt's OWN
// listing tags BOTH as `.fn fn_800C3B20, global` / `.fn fn_800C3B60, global`
// (lines 3109, 3128) -- real symbol names, global binding, not stripped.
// Left as `static` here anyway because flipping linkage is a whole-TU
// concern (symbol table / relocation visibility for the eventual link, not
// an instruction-content one) and is out of scope for a per-function content
// fix; flagging it rather than silently reconciling it.
// ===========================================================================
...
/// on the upper side).
///
/// Register-permutation residual, MEASURED not merely reasoned: originally
/// coded as a single flat `if (self->mUnitBasePos.x > self->mPos.x) { ... }`
/// (matching the equally-plausible mirror below), which reproduced retail's
/// REGISTERS (f1=mUnitBasePos.x, f0=mPos.x) exactly but with the fcmpo
/// operand slot and branch mnemonic reversed (`fcmpo f1,f0;ble` vs target's
/// `fcmpo f0,f1;bge`) -- a pure slot/polarity swap, not a register problem,
/// on a comparison between TWO MEMBERS with no literal involved at all.
/// Rewriting to the mirrored `if (self->mPos.x < self->mUnitBasePos.x)`
/// (direct `<`, operand order flipped) fixes the slot/polarity but then
/// mis-numbers the REGISTERS (mUnitBasePos.x drops to f0, mPos.x takes f1) --
/// confirming levers 12/13 apply independently of the slot-order lever.
/// mUnitBasePos.x is read again a few lines later (both in the `upper`
/// calculation and reused again after that), so per lever 13 ("a member READ
/// and reused wants its own named local") it gets `baseX`; giving mPos.x ITS
/// OWN named locals at BOTH occurrences (`posX`, and a fresh SECOND local
/// `posX2` after the branch join, per lever 13's second half) is what
/// finally reproduces retail's exact register set. The two-argument
/// occurrences of `self->mPos.x` bare, without their own locals, kept
/// forcing an extra reload (16 words, one over target) or the wrong
/// register for `baseX` -- only naming BOTH bare reads closes it at the
/// correct 15 words. `upper`'s slot order (`!(posX2 >= upper)` rather than
/// `posX2 < upper`) is lever 7: the direct, un-negated `>=` is what lowers
/// to the `cror`+`bne`-to-return (`bnelr`) shape target has; the negated-`<`
/// spelling instead emits the fast, cror-free branch, which is wrong here.
...
/// the Y axis, but the two are NOT byte-mirrors of each other in source
/// shape, measured: fn_800C3B20's second comparison needs a NAMED local for
/// the re-read of `self->mPos.x` (`posX2`) to hit 15 words; giving
/// `self->mPos.y`'s second read the equivalent named local here (`posY2`)
/// instead COSTS a word (16, an extra reload) and additionally knocks
/// `baseY` from its correct register (f2) down to f1. Every combination of
/// named-vs-bare for the second read and compound-vs-flat for the `- 16.0f`
/// subtraction was tried; only leaving BOTH occurrences of `self->mPos.y <
/// baseY - 16.0f` written out as a repeated bare expression (relying on -O4
/// to CSE the two identical `baseY - 16.0f` subtractions across the
/// condition and the store, the same way it already CSEs the bare
/// `self->mUnitBasePos.y` reads across the branch above it) reproduces
/// target's register set exactly. Naming the shared subtraction result
/// (`f32 lower = ...`) is exactly what over-elevates its register priority
/// and produces the wrong permutation, mirroring fn_800C3B20's opposite
/// finding -- these two "identical-looking" clamp helpers want opposite
/// naming strategies for their second condition, which only measurement
/// caught.
