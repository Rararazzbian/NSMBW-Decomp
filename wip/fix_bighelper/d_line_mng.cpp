...
/// start_line_move() in the target's .text address order. No caller exists
/// in this merge (its real caller is presumably the un-authored fn_800C31C0),
/// so as an unreferenced `static` it is dead-stripped by -O4 and will not be
/// emitted here -- see MERGE.md, this is an expected, reported regression
/// against author_core's isolated (dummy-forced) measurement, not a defect.
...
// mOldPos` READS only; MAPPING.md's "first writer of the 0x58 field" claim
// does NOT hold for this function specifically -- see RESULT.md).
//
...
// NOT empty in target (MERGE.md's "empty in target" note was wrong for this
// one specific triple member -- init/finalize ARE empty, execute is not).
// This is fn_800C31C0's REAL caller: a plain `b fn_800C31C0` tail call at the
// very end, confirmed at target VA 0x800C562C. Gravity constants read from
// original/wiimj2d.dol .sdata2: -0.0625f @ 0x8042CB90 (per-frame accel),
// -4.0f @ 0x8042CB94 (terminal velocity clamp).
