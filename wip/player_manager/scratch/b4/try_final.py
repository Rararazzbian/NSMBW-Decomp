...
daYoshi_c *daPyMng_c::getYoshi(int plrNo) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (base != nullptr) {
            GetPlrNoFn fn = *(GetPlrNoFn *)((u8 *)(*(void **)((u8 *)base + 0x60)) + 0x6c);
...
"""
variants['v_b_castR3first'] = """
daYoshi_c *daPyMng_c::getYoshi(int plrNo) {
    fBase_c *base;
    for (int i = 0; i < 4; i++) {
        base = fManager_c::searchBaseByID((fBaseID_e)m_yoshiID[i]);
        if (base != nullptr) {
            dActor_c *actor = (dActor_c *)base;
            GetPlrNoFn fn = *(GetPlrNoFn *)((u8 *)(*(void **)((u8 *)actor + 0x60)) + 0x6c);
            if (fn(actor) == plrNo) {
                return (daYoshi_c *)base;
            }
        }
