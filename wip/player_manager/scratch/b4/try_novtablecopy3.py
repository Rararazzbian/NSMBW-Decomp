...
daYoshi_c *daPyMng_c::getYoshi(int plrNo) {
    ...
    for (int i = 0; i < 4; i++) {
        base = fManager_c::searchBaseByID((fBaseID_e)m_yoshiID[i]);
        if (base != nullptr) {
            GetPlrNoFn *vtbl = *(GetPlrNoFn **)((u8 *)base + 0x60);
            if (vtbl[0x6c / 4]((dActor_c *)base) == plrNo) {
                return (daYoshi_c *)base;
            }
        }
    }
...
daYoshi_c *daPyMng_c::getYoshi(int plrNo) {
    ...
    for (int i = 0; i < 4; i++) {
        base = fManager_c::searchBaseByID((fBaseID_e)m_yoshiID[i]);
        if (base != nullptr) {
            u32 vt = *(u32 *)((u8 *)base + 0x60);
            GetPlrNoFn fn = *(GetPlrNoFn *)(vt + 0x6c);
