...
public:
    ...
    //
    // SHADOW COPY (B2 scratch): `create`'s return type below is `bool`, not
    // the real header's `void`. See wip/player_manager/BATCH2.md -- every
    // exit path of create() (0x8005EEE0) explicitly loads r3 with 0 or 1,
    // the same evidence pattern that already flipped fn_8005f4d0. This file
    // is NOT the real header; the real header is untouched.
    // ---------------------------------------------------------------------
