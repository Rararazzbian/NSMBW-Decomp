...
TARGET_FILES = [
    'wip/kokoopa_verify5/auto_03_800A8710_text.txt',
    'wip/kokoopa_verify5/auto_sinit_text.txt',
    'wip/kokoopa_verify5/auto_03_800B03D8_text.txt',
]
DRAFT_SRC = sys.argv[1] if len(sys.argv) > 1 else 'wip/kokoopa_verify5/d_enemy_toride_kokoopa.cpp'
PREFIX = sys.argv[3] if len(sys.argv) > 3 else 'wip/kokoopa_verify5'
DRAFT_OBJ = f'{PREFIX}/draft.o'
DRAFT_DIS = f'{PREFIX}/draft_disasm.txt'
EXTRA_INC = [sys.argv[2]] if len(sys.argv) > 2 else ['wip/kokoopa_verify5/include']

HI = 0x800B0A20

def get_tu_syms():
    out = []
    with open('wip/kokoopa_verify5/target_list.txt', encoding='utf-8') as f:
        for line in f:
