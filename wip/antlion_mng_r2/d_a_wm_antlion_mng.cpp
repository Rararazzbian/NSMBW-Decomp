...
/// @unofficial ROUND r2: 39 -> 37 differing. The one lever that moved was REMOVING the
/// `int cand = candidates[randIdx];` local -- with the local, `cand` gets a def-point and takes
/// r3 while the scaled index goes to r0; retail has them the other way round, which is what a
/// bare `candidates[randIdx]` expression produces (lever 13 applied in reverse). Three residuals
/// remain, all measured:
///   * out/candidates/playerPoint occupy a 3-CYCLE of r22/r23/r29 against retail's r23/r29/r22.
///     These are registers RE-USED after phase 1's live ranges end, so the assignment is a
///     live-range property, not a declaration-order one; declaration order, reference-vs-pointer
///     for the phase-3 map, and an explicit `int *outPtr = out;` were all byte-identical.
///   * The same for the phase-3 `map`/`j` pair (r26/r23 against retail's r23/r26).
///   * We are 2 instructions SHORT: retail tests `excludeCurrent` TWICE
///     (`beq A; cmpw pp,cand; bne B; A: cmpwi excl,0; bne next; B: store`). Six spellings of the
///     condition -- nested ifs, De Morgan, `continue` guards, a `bool skip` flag, a doubled
///     conjunct, and the original `&&`/`||` -- were measured this round; four are BYTE-IDENTICAL
///     and two are worse. This is a genuine optimiser wall, now confirmed on ten spellings total.
///
/// @unofficial Full reconstruction, read directly off fn_2_15BA70's own disassembly
...
/// @unofficial fn_2_15BC30: 29 -> 22 differing in round r2, by applying the first two thirds of
/// the rebuildAllModels recipe -- reference `map` plus `int accum;` declared first, with
/// `for (route = 0, accum = 0; ...)`. That put `map`(r25), `info`(r24) and `route`(r23) on
/// retail's registers exactly. Two residuals remain, both characterised:
///   * `accum` and the compiler's `&picked[i]` induction pointer are EXCHANGED (r26/r27 against
///     retail's r27/r26). Unlike the clearAllModels case these are not both plain locals -- one
///     is compiler-generated -- and 8 declaration/scope permutations moved it not at all.
///   * The `mVec3_c` stack slots are swapped: retail's GetPos return temp is at 0x24 and `pos`
///     at 0x18; we get 0x18/0x24. MEASURED NEGATIVE: `pos` declared at function scope, at outer
///     loop scope, at inner loop scope, before `picked`, after `antlion`, and as a second NAMED
///     local (`mVec3_c t = map.GetPos(...); pos = t;`) all produce BYTE-IDENTICAL output. The
///     GetPos destination always takes the LOW slot. `mVec3_c pos = map.GetPos(...)` does move
///     it, by eliding the copy entirely (6 words short) -- so that is not the answer either.
...
/// @unofficial fn_2_15BDA0: BYTE-IDENTICAL as of round r2. Closed by three stacked source
/// shapes, each measured on its own (8 -> 8 -> 2 -> 0):
///   1. `daWmMap_c &map = *daWmMap_c::m_instance;` -- a REFERENCE, not a pointer. A pointer
///      local initialised from a singleton is unconditionally PROMOTED to the highest
///      callee-saved GPR by MWCC; a reference is not, and retail has `map` mid-file (r29 here,
///      r29 in clearAllModels). All 24 declaration-order permutations leave a pointer in r31.
///   2. The accumulator `base` declared FIRST, so it takes the register the promotion had
///      been stealing.
///   3. `for (slot = 0, base = 0; ...)` with both declared above the loop, so `slot = 0` is
///      emitted BEFORE `base = 0`. That is a pure SCHEDULE effect and is independent of (2):
///      `int base = 0;` above the loop gets the register right and the schedule wrong (2 diffs).
/// Also confirmed unchanged from the earlier round: `setAntlion`'s receiver is a FRESH
/// `daWmMap_c::m_instance` read, and its two bools are this function's own two parameters.
...
/// @unofficial fn_2_15BE80: BYTE-IDENTICAL as of round r2, by the identical three-part recipe
/// documented on rebuildAllModels above (reference `map`, accumulator declared first,
/// `for (slot = 0, base = 0; ...)`). Measured path 8 -> 6 -> 2 -> 0. Two other spellings of
/// step 3 close it equally: a `while` loop with `int slot = 0; base = 0;` written in that
/// order, and `for (; slot < 2; slot++)` with an empty init.
/// Retained from the earlier round: GetMapEnemyInfo/SetMapEnemyInfo's first argument is the
/// unnamed `daWmMap_c` +0x3388 field, and `idx` is an outer-loop accumulator, not `sub + slot*2`.
