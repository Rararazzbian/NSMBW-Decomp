...
/* @unofficial Missing from the landed NANDOpenClose.h. Signatures inferred from
 * dNandThread_c::save()/load() call sites: NANDSimpleSafeOpen(path, info,
 * accessType, stagingBuffer, stagingBufferSize). PROPOSED for the real header. */
s32 NANDSimpleSafeOpen(const char* path, NANDFileInfo* info, u8 access, void* buffer, u32 bufferSize);
