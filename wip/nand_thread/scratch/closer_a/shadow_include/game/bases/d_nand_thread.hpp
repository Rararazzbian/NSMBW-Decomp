...
public:
    ...
    /// @unofficial No name in the symbol map (0x800CF170); cmdSave per
    /// Batches 2/3 (mState=4 matches run()'s save dispatch, plus the
    /// l_tmpSave memcpy). Not in the real header -- shadow-only, for this
    /// regression harness's sake.
    ...
    /// @unofficial `volatile`: proven necessary by direct A/B compile, not
    /// merely plausible because a background thread mutates it. See
    /// CLOSE_A.md -- without this qualifier, MWCC folds every
    /// `bool ok = (mError == OP); if (ok)` test back down to a plain `cmpwi`,
    /// regardless of source shape; with it, the identical source produces the
    /// target's `cntlzw`+`srwi.` 0/1-materialisation idiom exactly.
