...
/// @note CLOSER_C EXPERIMENT: the ctor now takes the OSMutex* to initialise.
/// The target's __ct__13dNandThread_cFiPQ23EGG4Heap interleaves the base's
/// vtable store, an OSInitMutex call, THEN mMutex's own vtable store, THEN
/// OSInitCond -- i.e. OSInitMutex happens strictly between the two vtable
/// stores. That is only reachable if OSInitMutex is called from INSIDE
/// EGG::Mutex's own constructor (which runs, and finishes, before mMutex's
/// own vptr-adjustment happens), not from mMutex's constructor body.
class Mutex {
public:
...
public:
    /// @note CLOSER_C EXPERIMENT: OSInitCond stays here, in mMutex's own body;
    /// only OSInitMutex moved to the base's ctor -- see EGG::Mutex above.
