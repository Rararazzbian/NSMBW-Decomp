...
public:
    ...
    bool check_term(); ///< @unofficial LOCAL TEST CHANGE (was void): the target calls
    /// it then immediately `cmpwi r3,0x0; beq ...` with no intervening load into r3 --
    /// the source must consume a return value here, so `void` cannot be right.
    /// Declared `bool` on the strength of `is_unit_circle2x2`/`is_unit_circle4x4`'s
    /// already-confirmed `li r3,0x1/0x0; blr` idiom elsewhere in this same class.
    /// Not yet cross-checked against a compile-both-ways register-allocation test --
    /// flagged for the merge to verify per AGENT_CONTEXT.md's CFront-mangling rule.
