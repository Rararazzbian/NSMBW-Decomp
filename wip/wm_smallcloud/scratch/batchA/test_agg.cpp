...
void daWmSmallCloud_c::createModel() {
    ...
    nw4r::g3d::ResMdl resMdl = mResFile.GetResMdl(sData.resMdlNames[ACTOR_PARAM(CourseNo)]);
    mModel.create(resMdl, &mAllocator, nw4r::g3d::ScnMdl::BUFFER_RESMATMISC, 1);

    static bool sInit = false;
    static const char *resAnmNames[4];
    if (!sInit) {
        resAnmNames[0] = sData.resMdlNames[0];
        resAnmNames[1] = sData.resMdlNames[1];
        resAnmNames[2] = sData.resMdlNames[2];
        resAnmNames[3] = sData.resMdlNames[3];
        sInit = true;
    }
